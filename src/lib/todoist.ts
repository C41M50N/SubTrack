...
export async function getProjects(api: TodoistApi): Promise<TodoistProject[]> {
  ...
  return projects.map((proj) => ({ 
    ...
    hex_color: TodoistColors.find((obj) => obj.name === (proj.color as TodoistColor))!.color 
  }));
}

export async function getProjectName(api: TodoistApi, project_id: string): Promise<string> {
  await sanityCheck(api);
...
export async function createReminder(api: TodoistApi, projectId: string, title: string, reminder_date: Date) {
  await sanityCheck(api);
  await api.addTask({
