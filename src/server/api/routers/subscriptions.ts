import { CreateSubscriptionSchema, MoveSubscriptionSchema, SubscriptionSchema } from '@/features/subscriptions';
import createSubscription from '@/features/subscriptions/actions/create-subscription';
import deleteSubscription, { DeleteSubscriptionProps } from '@/features/subscriptions/actions/delete-subscription';
...
export const subscriptionsRouter = createTRPCRouter({
  ...
  moveSubscription: protectedProcedure.input(MoveSubscriptionSchema).mutation(async ({ ctx, input }) => {
    return await moveSubscription(ctx, input);
  }),

  deleteSubscription: protectedProcedure.input(DeleteSubscriptionProps).mutation(async ({ ctx, input }) => {
