...
export function useCollections() {
  const { data: collections, isLoading: isGetCollectionsLoading } = api.collections.getCollections.useQuery(undefined, {
...
export function useCreateCollection() {
  ...
  const { mutateAsync: createCollection, isLoading: isCreateCollectionLoading } =
...
export function useEditCollectionTitle() {
  const utils = api.useUtils();
  const { mutateAsync: editCollectionTitle, isLoading: isEditCollectionLoading } =
